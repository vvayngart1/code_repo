#include "instrument_loader_legacy.h"
...
bool checkRequireFields(const OnixS::CME::MarketData::Message& message) {
    if ( !message.contain(tag_CFICode) )
        return false;                

    if ( !message.contain(tag_UnderlyingProduct) )
        return false;

    if ( !message.contain(tag_SecurityExchange) )
        return false;
    
    if ( !message.contain(tag_SecurityDesc) )
        return false;

    if ( !message.contain(tag_SecurityGroup) )
        return false;

    if ( !message.contain(tag_MinPriceIncrement) )
        return false;
    
    if ( !message.contain(tag_MinPriceIncrementAmount) )
        return false;
    
    if ( !message.contain(tag_UnitOfMeasuryQty) )
        return false;
    
    if ( !message.contain(tag_DisplayFactor) )
        return false;
    
    if ( !message.contain(tag_NoEvents) )
        return false;
    
    if ( !message.contain(tag_SecurityId) )
        return false;
...
void InstrumentLoader::onSecurityDefinition(const OnixS::CME::MarketData::SecurityDefinition& securityDefinition,
    ...
    try {
        ...
        const OnixS::CME::MarketData::Message& message = securityDefinition.underlyingMessage();
        if ( !checkRequireFields(message) )
            return;
        
        if ( message.contain(tag_NoLegs) && message.getInteger(tag_NoLegs) > 1 )
            return;
        ...
        temp = message.get(tag_CFICode);
        if ( temp.empty() || temp[0] != 'F' )
            return;
        
        const std::string& displayName = securityDefinition.getDescription();
        bool insert = false;
        ...
        if ( !instrumentPtr ) {
            ...
            instrumentPtr->_keyId = message.getInteger(tag_SecurityId);
        ...
        
        {
            ...
            _securityIdToDisplayName[message.getInteger(tag_SecurityId)] = message.get(tag_SecurityDesc);
        }
            
        instrument->_CFICode = temp;
            
        switch (message.getInteger(tag_UnderlyingProduct)) {
            case 2:
        ...
        instrument->_contractSize = message.getDouble(tag_UnitOfMeasuryQty);
        
        double tickSize = message.getDouble(tag_MinPriceIncrement);
        ...
        if ( group ) {
            for ( size_t index = 0; index < group->numberOfEntries(); ++index ) {
                if ( group->contain(tag_EventDate, index) && group->get(tag_EventDate, index).length() > 7 ) {
                    temp = group->get(tag_EventDate, index);
                    temp += " ";
                    if ( group->contain(tag_EventTime, index) && group->get(tag_EventTime, index).length() > 8 )
                        temp += group->get(tag_EventTime, index);
                    else
                        temp += "000000000";
                    ...
                    temp.insert(16, ":");
                    temp.insert(19, ".");
                    switch ( group->getInteger(tag_EventType, index) ) {
                        case 5:
                    ...
                    LOGGER_WARN << "displayName: " << displayName
                                << ",tag_EventDate=" << (group->contain(tag_EventDate, index) ? group->get(tag_EventDate, index) : "<NULL>")
                                << ",tag_EventTime=" << (group->contain(tag_EventTime, index) ? group->get(tag_EventTime, index) : "<NULL>")
                                << " are NOT in right format for index: " << index << " out of: " << group->numberOfEntries() << "\n";
                }
            }
        }
        
        temp = message.get(tag_SecurityExchange);
        if ( temp == "XCBT" )
        ...
        instrument->_keyNum1 = instrument->_keyNum2 = message.getInteger(tag_SecurityId);
        instrument->_keyStr1 = instrument->_keyStr2 = message.get(tag_SecurityId);
        
        instrument->_var1 = channelId;
        instrument->_instrType._enum = tw::instr::eInstrType::kFuture;
        
        instrument->_displayFormat = message.getDouble(tag_DisplayFactor);
        
        instrument->_productCode = message.get(tag_SecurityGroup);
        
        // NOTE: CBOT interest rate (Symbol(55)=ZB) futures and agricultural futures for corn (Symbol(55)=ZC),
        // soybeans (Symbol(55)=ZS), wheat (Symbol(55)=ZW), oats (Symbol(55)=ZO) don't conform to formula:
        ...
        // by CME field
        //
        // Also, need to get Symbol(tag 55) from SecurityDefinition, because for
        // CBOT interest rate futures it is 'ZB', even though actual future can be
        // ZT, ZF, ZN, or ZB
        //
        instrument->_symbol =  securityDefinition.getSymbol();
        instrument->_tickValue = tickSize * instrument->_contractSize * instrument->_displayFormat;
        if ( "ZB" == instrument->_symbol || "ZC" == instrument->_symbol || "ZS" == instrument->_symbol || "ZW" == instrument->_symbol || "ZO" == instrument->_symbol )
            instrument->_tickValue /= 100;
        else if ( "GE" == instrument->_symbol )
            instrument->_tickValue = message.getDouble(tag_MinPriceIncrementAmount);
        
        instrument->_numberOfLegs = 0;        
        if ( message.contain(tag_NoLegs) ) {
            // TODO: currently support spreads with only up to 4 legs
            //
            group = message.getGroup(tag_NoLegs);
            if ( group && group->numberOfEntries() <= 4 ) {
                instrument->_instrType._enum = tw::instr::eInstrType::kSpread;
                instrument->_numberOfLegs = group->numberOfEntries();
                for ( size_t index = 0; index < group->numberOfEntries(); ++index ) {
                    temp = group->get(tag_LegSecurityId, index);
                    int32_t ratio = ( group->getInteger(tag_LegSide, index) != 2 ) ? group->getInteger(tag_LegRatioQty, index) : -1 * group->getInteger(tag_LegRatioQty, index);
                    switch ( index ) {
                        case 0:
        ...
        OnixS::CME::MarketData::MessageFormattingFlags options = OnixS::CME::MarketData::MessageFormattingOptions::IncludeTagNumber | OnixS::CME::MarketData::MessageFormattingOptions::IncludeTagName;
        LOGGER_INFO << "FIX Message: " << message.toString(options, '\n') << "\n";
        ...
        os << "MinPrIncr(969): " << std::setw(10) << std::right <<  message.get(tag_MinPriceIncrement);
        os << std::setw(30) << std::right;
        os << "UnitOfMsrQty(1147): " << std::setw(10) << std::right <<  message.get(tag_UnitOfMeasuryQty);
        os << std::setw(30) << std::right;
        os << "MinPrIncrAmnt(1146): " << std::setw(10) << std::right <<  message.get(tag_MinPriceIncrementAmount);
        os << std::setw(20) << std::right;
        os << "DispFactr(9787): " << std::setw(10) << std::right <<  message.get(tag_DisplayFactor);
        os << "\n";
        os << "MinPrIncr(969)*UnitOfMsrQty(1147)*DispFactr(9787): ";
        os << message.get(tag_MinPriceIncrement) << "*";
        os << message.get(tag_UnitOfMeasuryQty) << "*";
        os << message.get(tag_DisplayFactor) << "=";
        os << (message.getDouble(tag_MinPriceIncrement)*message.getDouble(tag_UnitOfMeasuryQty)*message.getDouble(tag_DisplayFactor));
    ...
    try {
        {
            ...
            if ( iter == _channelsStatus.end() ) {
                LOGGER_ERRO << "Can't find channelId: " << channelId << "\n";
                return;
            }
