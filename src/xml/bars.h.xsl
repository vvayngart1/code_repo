...
<xsl:template match="Namespace/BarsDefs" mode="enum">
<xsl:for-each select="*">
    <xsl:choose>
...
<xsl:template match="Namespace/BarsDefs" mode="fwd_decl">
<xsl:for-each select="*">
    <xsl:choose>
...
<xsl:template match="Namespace/BarsDefs" mode="callbacks">
<xsl:for-each select="*">
    <xsl:choose>
...
<xsl:template match="Namespace/BarsDefs" mode="decl">
<xsl:for-each select="*">
    <xsl:choose>
        <xsl:when test="@type='struct'">
...
struct  <xsl:value-of select="name()"/> {
        ...
        try {
            ...
            <xsl:if test="Filters">
                ...
                <xsl:for-each select="Filters/*">
                    <xsl:variable name = "x"><xsl:value-of select="@value"/></xsl:variable> 
                    <xsl:variable name = "i"><xsl:value-of select="position()"/></xsl:variable> 
                    <xsl:for-each select="../../Params/*">
                        <xsl:if test="position() = $i">
                            <xsl:if test="position() != 1"> AND </xsl:if>
                            <xsl:value-of select="substring-before($x, '?')"/><xsl:text>"</xsl:text>
                            <xsl:text> &lt;&lt; </xsl:text><xsl:value-of select="name()"/>
                            <xsl:text> &lt;&lt; "</xsl:text><xsl:value-of select="substring-after($x, '?')"/>
                         </xsl:if>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:if>   
    ...
    <xsl:for-each select="Outputs/*">
       <xsl:text>std::vector&lt;tw::common_trade::</xsl:text>
       <xsl:value-of select="name()"/>
