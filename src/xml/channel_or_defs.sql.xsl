...
/*!40101 SET character_set_client = utf8 */;
    ...
    <xsl:text>&#10;&#9;`index` int(11) NOT NULL AUTO_INCREMENT,</xsl:text>
     ...
     <xsl:for-each select="/Namespace/OrdersDefs/*">        
        <xsl:if test="$tableField = name()">
            ...
            <xsl:for-each select="*[not(@serializable='false')]">
                ...
                <xsl:value-of select="name()"/>
                <xsl:text>` </xsl:text>
                <xsl:call-template name="TableFieldType">
                    <xsl:with-param name="table" select="$table" />
...
<xsl:template name="TableFieldType">
    <xsl:param name="table"/>
    <xsl:param name="tableField"/>
    <xsl:param name="item"/>
    
    <xsl:for-each select="/Namespace/OrdersDefs/*">        
        <xsl:if test="$tableField = @parent">
            <xsl:call-template name="TableFieldType">
                <xsl:with-param name="table" select="$table" />
                <xsl:with-param name="tableField" select="name()" />
    ...
    <xsl:for-each select="/Namespace/SQLDefs/Tables/*">
        <xsl:if test="$table=name()">
            <xsl:if test="count(Fields/*[name()=$tableField]) != 0">
                <xsl:choose>
                    <xsl:when test="Fields/*[name()=$tableField]/*[name()=$item]/@type">
                        <xsl:value-of select="Fields/*[name()=$tableField]/*[name()=$item]/@type"/>
                    </xsl:when>
                    <xsl:otherwise>
